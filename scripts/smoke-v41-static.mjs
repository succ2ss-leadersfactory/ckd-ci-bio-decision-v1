...
for (const marker of ['V41PreviewApp', 'V41PromptPracticeReviewLab', 'V41ResearchStrategyTrimmedLab', 'v41 research strategy lab cloned', 'V41_VISIBLE_APP_STEPS', 'V41FlowStrip', 'V41ProgressCoachPanel', 'ckd.v41.pharmaStrategyResearch.v1']) mustInclude(files.app, marker, 'v41 app');
for (const marker of ['V40VNextPromptPracticeReviewLab', 'V40VNextResearchStrategyTrimmedLab', "import './journey-v40-vnext-app-preview'"]) mustNotInclude(files.app, marker, 'old app dependency');
...
for (const marker of ['V41_VISIBLE_APP_STEPS', 'v41 field-friendly step labels', '시장 변화 읽기']) mustInclude(files.config, marker, 'v41 config');
for (const marker of ['V41FlowStrip', 'v41 field-friendly flow chips', '시장 읽기']) mustInclude(files.ux, marker, 'v41 ux');
...
for (const marker of ['src/journey-v41-research-strategy-trimmed-lab.tsx', 'src/journey-v41-pharma-strategy-research-lab.tsx', 'src/journey-v41-pharma-research-data.ts', 'src/journey-v41-pharma-research-prompts.ts', 'src/journey-v41-pharma-research-parser.ts']) mustInclude(files.tsconfig, marker, 'v41 tsconfig');
