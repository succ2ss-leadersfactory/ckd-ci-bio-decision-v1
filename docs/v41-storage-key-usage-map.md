...
7. `ckd.v41.taskManagement.v10` now carries the pure task-management chain: Step 6 work output and work breakdown → Step 7 execution sequence, responsibility, checkpoint, workload adjustment, and instruction draft → Step 8 task boundary, bottleneck response, and escalation criteria.
...
- Step 6 must use `review` as the human-reviewed handoff summary.

## Step 6 work-output and work-breakdown rule
...
→ Step 6 work output, output location, completion standard, KPI connection check, CSF reflection check, work breakdown
...
Step 6 may read `ckd.v41.performanceCascade.aiExpansion.v1`, but the AI expansion must remain candidate/reference material.
...
→ Step 6 reviewed reference for work-output/work-breakdown only
