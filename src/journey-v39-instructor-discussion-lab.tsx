...
function buildInstructorDiscussionGuide(result: V39FinalCallPlanResult) {
  ...
  return [
    ...
    '3. 고객군 × 팀원 실행 Map의 배치 기준 질문: 고객군 대응 방향과 팀원 연결 기준은 어떤 근거로 연결되었는가?',
    '4. 팀원 실행 대화의 첫마디 질문: 팀원에게 말할 첫 문장은 지시가 아니라 역할 기준과 지원 방식을 담고 있는가?',
    '5. 컴플라이언스 수정 포인트 질문: AI 초안에서 어떤 표현을 수정했고, 왜 더 안전한 문장으로 바꾸었는가?',
    '6. 현업 적용 2주 후 리뷰 질문: 2주 뒤 무엇을 보면 이 실행계획이 작동했는지 판단할 수 있는가?',
  ].join('\n');
}
...
function buildDiscussionQuestions(result: V39FinalCallPlanResult): DiscussionQuestion[] {
  return [
    ...
    {
      title: '고객군 × 팀원 실행 Map의 배치 기준',
      body: result.memberRoles || '팀원별 역할이 고객군 대응 방향, 팀원 강점, 부담 가능성, 성장 기회를 함께 반영했는지 확인합니다.',
...
function buildTwoWeekReviewQuestions(result: V39FinalCallPlanResult) {
  return [
    ...
    '3. 고객군 × 팀원 실행 Map에서 부담이 특정 팀원에게 몰리지는 않았는가?',
    '4. 팀원에게 말한 첫 문장은 역할 기준과 지원 방식을 충분히 담았는가?',
    '5. AI 초안에서 수정한 안전 문장이 실제 현장에서 도움이 되었는가?',
    `6. 다음 2주에 유지할 것과 바꿀 것은 무엇인가? ${result.twoWeekAction.trim() ? '현재 실행 우선순위와 비교해 답합니다.' : ''}`.trim(),
  ];
}
  ...
  return (
    <section className="rounded-3xl border border-cyan-100 bg-cyan-50 p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-700">
            ...
            이 화면은 참여자를 점수화하거나 평가하기 위한 화면이 아니라, 판단 근거·포기한 선택지·고객 Data 부족 정보·팀원 실행 대화·안전 문장 수정 이유·2주 후 리뷰 기준을 토의하기 위한 운영 화면입니다.
          </p>
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-cyan-200 bg-white p-4 text-xs font-bold leading-5 text-cyan-950">
        ...
        <p className="mt-1">질문은 정답 확인용이 아니라 판단 기준을 꺼내는 용도입니다. “왜 그렇게 보았는가”, “무엇을 아직 모르는가”, “어떤 표현을 고쳤는가”, “2주 후 무엇으로 확인할 것인가”를 중심으로 운영합니다.</p>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold leading-5 text-amber-950">
        이 요약은 참여자 평가용 점수 자료가 아닙니다. 강사는 관리 지표 선택 이유, 고객 Data 확인 List의 부족 정보, 고객군 × 팀원 실행 Map의 배치 기준, 팀원 실행 대화의 첫마디, 컴플라이언스 수정 포인트, 현업 적용 2주 후 리뷰 질문을 중심으로 토의합니다.
      </div>
    </section>
  );
}
