...
function buildInstructorDiscussionGuide(result: V39FinalCallPlanResult) {
  if (!hasFinalCardContext(result)) {
    return [
      ...
      '12단계에서 먼저 볼 고객군/조건, 코칭 대상·실행 대화 요약, 이번 2주에 할 일, 말해도 되는 선, 팀원에게 꺼낼 첫 문장을 먼저 저장하세요.',
    ].join('\n');
  }
  ...
  return [
    ...
    `- 코칭 대상·실행 대화 요약: ${result.memberRoles || '아직 작성되지 않았습니다.'}`,
    ...
    '3. 고객군별 2주 대응 방향은 어떤 Data와 부족 정보에 근거했는가?',
    '4. 왜 이 팀원을 먼저 1on1로 만나야 하며, 대화 목적은 무엇인가?',
    '5. 선택 이유와 코칭 초점은 관찰 가능한 신호에 기반했는가, 아니면 성격 단정에 가까운가?',
...
function buildDiscussionQuestions(result: V39FinalCallPlanResult): DiscussionQuestion[] {
  return [
    ...
    {
      title: '고객군별 2주 대응 방향의 근거',
      body: result.focusCustomers || '고객군별 2주 대응 방향이 고객 Data, 부족 정보, 다음 확인 질문과 어떻게 이어지는지 확인합니다.',
      guide: '누구에게 맡길 것인가가 아니라, 어떤 신호를 보고 어떤 방향으로 움직이기로 했는지 묻습니다.',
    },
    {
      title: '먼저 1on1로 맞춰볼 이유',
      body: result.memberRoles || '먼저 만날 팀원, 선택 이유, 대화 목적, 코칭 초점, 단정하면 안 되는 해석을 확인합니다.',
      guide: '먼저 만날 이유가 분명한지, 대화 목적이 부담 확인인지, 성장 의미 부여인지, 실행 기준 합의인지 구분되어 있는지 봅니다.',
    },
    {
      title: '관찰 신호와 해석의 구분',
      body: result.memberRoles || '코칭 대상 선정이 관찰 가능한 신호에 기반했는지, 성격·세대·태도 단정으로 흐르지 않았는지 이야기합니다.',
      guide: '“MZ라서”, “경력자니까”, “소극적이라서” 같은 단정 대신 실제 행동 신호와 확인 질문으로 표현했는지 확인합니다.',
    },
    {
      title: '팀원에게 꺼낼 첫 문장',
      body: result.firstMessage || '팀장의 첫마디가 지시나 압박이 아니라 대화 목적, 확인할 신호, 지원 방식을 분명히 담고 있는지 이야기합니다.',
...
function buildTwoWeekReviewQuestions(result: V39FinalCallPlanResult) {
  return [
    ...
    '3. 고객군별 2주 대응 방향은 실제 고객 반응 신호와 맞았는가?',
    '4. 먼저 만나기로 한 팀원과 실제로 대화했는가? 대화 목적은 유지되었는가?',
    '5. 코칭 대상 선정 과정에서 관찰 신호와 해석을 구분했는가?',
    '6. 팀원에게 꺼낸 첫 문장은 이유, 기준, 지원 방식을 충분히 담았는가?',
  ...
  return (
    <section className="rounded-3xl border border-cyan-100 bg-cyan-50 p-5 shadow-sm md:p-6">
      ...
      <div className="mt-4 rounded-2xl border border-cyan-200 bg-white p-4 text-xs font-bold leading-5 text-cyan-950">
        ...
        <p className="mt-1">질문은 정답을 맞히기 위한 것이 아닙니다. “왜 그렇게 보았는가”, “무엇을 아직 모르는가”, “왜 이 팀원을 먼저 만나야 하는가”, “관찰 신호와 해석을 구분했는가”, “어떤 표현을 고쳤는가”, “2주 후 무엇으로 확인할 것인가”를 중심으로 이야기합니다.</p>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold leading-5 text-amber-950">
        이 메모는 참여자 평가용 점수 자료가 아닙니다. 강사는 이번 2주 동안 볼 지표, 방문·면담 기록에서 아직 모르는 것, 고객군별 2주 대응 방향의 근거, 먼저 1on1로 맞춰볼 이유, 관찰 신호와 해석의 구분, 팀원에게 꺼낼 첫 문장, 말해도 되는 선을 고친 이유, 2주 뒤 확인 질문을 중심으로 대화를 이끕니다.
      </div>
    </section>
  );
}
