...
function TaskSummary() {
  ...
  return (
    <section className="rounded-3xl border bg-white p-5 shadow-sm md:p-6">
      <p className="text-sm font-black text-slate-950">9·10단계 실행 결과 다시 보기</p>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black text-slate-500">먼저 할 일</p><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{asList(taskState.selectedPriorityTasks)}</p></div>
        <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black text-slate-500">잠시 줄일 일</p><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{asList(taskState.selectedReduceTasks)}</p></div>
        <div className="rounded-2xl bg-slate-50 p-4 lg:col-span-2"><p className="text-xs font-black text-slate-500">업무 흐름 3단계</p><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{[taskState.flowStepOne, taskState.flowStepTwo, taskState.flowStepThree].filter(Boolean).join(' → ') || '미작성'}</p></div>
        <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black text-slate-500">팀장 확인 필요</p><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{taskState.leaderCheckWork || '미작성'}</p></div>
        <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black text-slate-500">부서 협조 필요</p><p className="mt-2 text-sm font-bold leading-6 text-slate-800">{taskState.crossFunctionalHelp || '미작성'}</p></div>
      </div>
    </section>
  );
}

export function V40VNextPeopleSelectionLab() {
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-indigo-100 bg-white p-5 shadow-sm md:p-6">
        ...
        <p className="mt-3 max-w-4xl text-sm font-bold leading-7 text-slate-600">가장 문제가 큰 팀원을 고르는 시간이 아닙니다. 이번 2주 실행 흐름을 실제로 움직이게 하기 위해 먼저 대화해야 할 팀원을 선택합니다.</p>
      </section>
      ...
      <section className="rounded-3xl border bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2">
          {behaviorOptions(member).map((behavior) => (
            <ChoiceButton key={behavior} selected={state.observedBehaviors.includes(behavior)} onClick={() => update({ observedBehaviors: toggleList(state.observedBehaviors, behavior) })}>{behavior}</ChoiceButton>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-3xl border bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2 lg:grid-cols-4">
          {RISKY_INTERPRETATIONS.map((risk) => (
            <ChoiceButton key={risk} selected={state.riskyInterpretations.includes(risk)} onClick={() => update({ riskyInterpretations: toggleList(state.riskyInterpretations, risk) })}>{risk}</ChoiceButton>
          ))}
        </div>
      </section>
