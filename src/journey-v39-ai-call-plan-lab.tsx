...
const V39_AI_CALL_PLAN_LAB_SMOKE_MARKERS = [
  'V39AiCallPlanLab',
  'V39CoachingTargetCallPlanPanel',
  'V39MemberRoleCallPlanPanel',
  '5단계 관리 지표',
  '6단계 고객 Data 확인 List',
  '7단계 고객군별 2주 대응 방향',
  '8단계 코칭 대상 선정 요약',
  '8단계 코칭 대상이 없어도 9단계 대화 메모를 반영합니다',
  '9단계 실행 대화 요약',
  '선택한 대화 목적 ID',
  '목적에 맞게 바꾼 첫마디',
  '코칭 대상별 실행 대화 포인트',
...
function buildCustomerStrategySummary(result: V39CustomerStrategyResult) {
  const strategies = Object.values(result.strategies).filter((strategy) => strategy.strategy.trim() || strategy.risk.trim() || strategy.priority.trim());
  if (strategies.length === 0) {
    return '7단계 고객군별 2주 대응 방향:\n- 아직 고객군별 2주 대응 방향이 저장되지 않았습니다.';
  }
...
function buildCoachingTargetSummary(result: V39TeamSevenCoachingMapResult) {
  const items = Object.values(result.decisions).filter(hasCoachingTargetContext);
  const priorityItems = items.filter((item) => item.priorityOneOnOne);

  if (items.length === 0) {
    return '8단계 코칭 대상 선정 요약:\n- 아직 우선 1on1 대상, 선택 이유, 코칭 초점이 저장되지 않았습니다.';
  }

  return [
    '8단계 코칭 대상 선정 요약:',
    `- 판단 메모: ${items.length}건`,
    `- 우선 1on1 대상: ${priorityItems.length > 0 ? priorityItems.map((item) => item.memberLabel).join(' · ') : '아직 선택되지 않았습니다.'}`,
    '',
    ...items.slice(0, 6).flatMap((item, index) => [
      `${index + 1}. ${item.memberLabel}`,
      `- 우선 1on1 포함: ${item.priorityOneOnOne ? '예' : '아니오'}`,
      `- 1on1 코칭 목적: ${item.coachingPurpose || '아직 정리되지 않았습니다.'}`,
      `- 선택 이유: ${item.selectionReason || item.leaderSupport || '아직 정리되지 않았습니다.'}`,
      `- 9단계로 넘길 코칭 초점: ${item.coachingFocus || '아직 정리되지 않았습니다.'}`,
      `- 단정하면 안 되는 해석/주의할 지점: ${item.riskMemo || '아직 정리되지 않았습니다.'}`,
      item.aiJudgmentDraft ? `- AI 판단 정리: ${item.aiJudgmentDraft}` : '',
      '',
    ]),
  ].join('\n');
}
...
function buildCallPlanContextPrompt(
  ...
  return [
    ...
    '- 팀원을 세대 특성으로 단정하지 말고, 관찰 신호·코칭 초점·지원 방식·책임 범위·실행 대화 규범을 함께 반영해 주세요.',
    '',
    ...
    '',
    buildCoachingTargetSummary(coachingTargetResult),
    '',
    '9단계 실행 대화 요약:',
    ...
    '3. 코칭 대상별 실행 대화 문장을 제안하되, 지시가 아니라 선택 이유·코칭 초점·지원 방식·확인 질문이 포함된 실행 대화 문장으로 작성해 주세요.',
...
function V39CoachingTargetCallPlanPanel({
  dashboardResult,
  ...
  onRefreshCoachingTargets: () => void;
  onRefreshPeopleDialogue: () => void;
}) {
  const savedCoachingTargets = Object.values(coachingTargetResult.decisions).filter(hasCoachingTargetContext);
  const priorityCoachingTargets = savedCoachingTargets.filter((item) => item.priorityOneOnOne);
  const savedCustomerStrategies = Object.values(customerStrategyResult.strategies).filter((strategy) => strategy.strategy.trim() || strategy.risk.trim() || strategy.priority.trim());
  const [copied, setCopied] = useState(false);
  const [callPlanItems, setCallPlanItems] = useState<Record<string, V39AiCallPlanResultItem>>(buildInitialCallPlanSaveState);
  const callPlanContextPrompt = useMemo(() => buildCallPlanContextPrompt(dashboardResult, customerJudgmentResult, customerStrategyResult, coachingTargetResult, peopleDialogueResult), [dashboardResult, customerJudgmentResult, customerStrategyResult, coachingTargetResult, peopleDialogueResult]);
  ...
  const applyCallPlanDraft = () => {
    updateCallPlanItem({
      callPlanDraft: currentCallPlan.callPlanDraft || '5단계 관리 지표, 6단계 고객 Data 확인 List, 7단계 고객군별 2주 대응 방향, 8단계 코칭 대상 선정, 9단계 실행 대화를 종합해 2주 실행계획 초안을 정리한다.',
      riskMemo: currentCallPlan.riskMemo || '처방 유도 표현, 비교 우위 단정, 허가 외 표현, 실제 고객·병원·의료진·처방 정보 포함 여부, 고객을 등급화하는 표현, 팀원을 세대 특성으로 단정하거나 부담을 전가하는 표현을 점검한다.',
      cleanupFocus: currentCallPlan.cleanupFocus || '11단계에서 승인자료 범위, 질문 중심 표현, 가상 고객군 기준, 코칭 대상별 실행 대화 기준으로 문장을 안전하게 수정한다.',
    });
  };
    ...
    <section className="rounded-3xl border border-sky-100 bg-sky-50 p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-3">
            ...
            <div className="rounded-2xl border border-sky-100 bg-white px-4 py-3 text-xs font-bold leading-5 text-slate-700">
              ...
              <p className="mt-1">5단계 관리 지표, 6단계 고객 Data 확인 List, 7단계 2주 대응 방향, 8단계 코칭 대상 선정, 9단계 실행 대화입니다.</p>
            </div>
            <div className="rounded-2xl border border-sky-100 bg-white px-4 py-3 text-xs font-bold leading-5 text-slate-700">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" className="rounded-full border bg-white px-4 py-2 text-sm font-black text-slate-700 shadow-sm" onClick={onRefreshCoachingTargets}>8단계 다시 불러오기</button>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm"><p className="text-xs font-black text-slate-500">7단계 대응 방향</p><p className="mt-1 text-sm font-black text-slate-900">{savedCustomerStrategies.length}건</p></div>
        <div className="rounded-2xl bg-white p-4 shadow-sm"><p className="text-xs font-black text-slate-500">8단계 코칭 대상</p><p className="mt-1 text-sm font-black text-slate-900">{priorityCoachingTargets.length > 0 ? `${priorityCoachingTargets.length}명 우선` : `${savedCoachingTargets.length}건`}</p>{coachingTargetResult.updatedAt ? <p className="mt-1 text-xs font-bold text-slate-500">{coachingTargetResult.updatedAt}</p> : null}</div>
      ...
      <div className="mt-3 rounded-2xl border border-sky-200 bg-white px-4 py-3 text-xs font-bold leading-5 text-sky-950">
        <p className="font-black">8단계 코칭 초점이 10단계 실행계획의 사람관리 기준이 됩니다</p>
        <p className="mt-1">우선 1on1 대상, 선택 이유, 코칭 목적, 단정하면 안 되는 해석 중 하나라도 저장되어 있으면 10단계 프롬프트 입력 맥락으로 가져옵니다. 업무를 배정하는 것이 아니라, 실행계획 안에 필요한 대화와 지원 조건을 반영합니다.</p>
      </div>
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-3">
        ...
        <article className="rounded-2xl border bg-white p-4 shadow-sm xl:col-span-1">
          <p className="text-sm font-black text-slate-950">7단계 고객군별 2주 대응 방향 요약</p>
      ...
      <div className="mt-4 rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm">
        <p className="text-sm font-black text-slate-950">8단계 코칭 대상 선정 요약</p>
        <pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap rounded-2xl bg-indigo-50 p-4 text-xs font-bold leading-6 text-indigo-950">{buildCoachingTargetSummary(coachingTargetResult)}</pre>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold leading-5 text-amber-950">
        10단계에서도 실제 고객명, 병원명, 의료진명, 제품명, 내부 매출·처방 수치, 개인정보는 입력하지 않습니다. AI에게 요청할 때도 교육용 가상 고객군, 코칭 대상, 실행 대화 맥락만 사용합니다.
      </div>
    </section>
  );
}
...
export function V39AiCallPlanLab() {
  ...
  return (
    <V39CoachingTargetCallPlanPanel
      ...
      onRefreshCoachingTargets={() => setCoachingTargetResult(loadV39TeamSevenCoachingMapResult())}
