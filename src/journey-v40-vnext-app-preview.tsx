...
type V40VNextParticipant = { groupName: string; tableName: string; representativeSituation: string; roleAccepted: boolean };
...
function V40ComplianceNotice() { return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><p className="font-black">AI에 넣기 전 확인할 것</p><p className="mt-1">실제 고객, 기관, 사람, 수치, 내부 전략처럼 식별되거나 민감한 정보는 가상·익명 표현으로 바꿉니다.</p></div>; }
...
function EntryStep({ participant, setParticipant }: { participant: V40VNextParticipant; setParticipant: (next: V40VNextParticipant) => void }) {
  return <div className="space-y-4"><V40VNextFlowStrip currentStep={1} /><V39StepHero eyebrow="1단계 · 팀장 역할 시작하기" icon="🤝" title="여러분은 C1바이오 영업팀장 역할로 시작합니다" tone="indigo" description="실제 활동은 팀 단위로 진행하지만, 화면에서의 역할은 영업팀장입니다. 여러분은 대표 상황을 정하고, 시장 변화와 팀원 실행 신호를 바탕으로 2주 실행 메모를 완성합니다." badges={[{ label: '운영 방식', value: '팀장 역할 실습', tone: 'indigo', icon: '🤝' }, { label: '참여 단위', value: '팀 활동', tone: 'emerald', icon: '👥' }, { label: '핵심 산출물', value: '2주 실행 메모', tone: 'amber', icon: '📝' }]} /><section className="rounded-3xl border border-indigo-100 bg-white p-4 shadow-sm md:p-5"><V39MiniFlow items={[{ icon: '🤝', title: '팀장 역할 잡기', body: 'C1바이오 영업팀장 관점에서 판단합니다.' }, { icon: '👥', title: '팀원 구성 이해', body: '다음 단계에서 이대호 팀장과 7명의 팀원을 확인합니다.' }, { icon: '📝', title: '대표 상황 정하기', body: '여러분이 다룰 대표 상황을 1개로 좁힙니다.' }]} /><div className="mt-3"><V39MinimumChecklist tone="indigo" items={['팀 선택', '이름/닉네임 입력', '대표 상황 1개 작성', '팀장 역할 확인']} /></div></section><V40ComplianceNotice /><ShellCard title="여러분이 다룰 대표 상황"><div className="grid gap-3 md:grid-cols-2"><label className="space-y-1"><span className="text-xs font-bold text-slate-500">팀</span><select className="w-full rounded-xl border bg-white px-3 py-2" value={participant.groupName} onChange={(event) => setParticipant({ ...participant, groupName: event.target.value })}><option value="">팀을 선택하세요</option>{TEAM_OPTIONS.map((team) => <option key={team} value={team}>{team}</option>)}</select></label><label className="space-y-1"><span className="text-xs font-bold text-slate-500">이름/닉네임</span><input className="w-full rounded-xl border bg-white px-3 py-2" value={participant.tableName} onChange={(event) => setParticipant({ ...participant, tableName: event.target.value })} placeholder="예: 김팀장, A테이블, 리더1" /></label></div><label className="block space-y-1"><span className="text-xs font-bold text-slate-500">여러분이 다룰 대표 상황</span><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={participant.representativeSituation} onChange={(event) => setParticipant({ ...participant, representativeSituation: event.target.value })} placeholder="예: 활동 기록은 늘었지만 고객 반응 이후 후속 실행이 약하고, 팀원별 기록 품질 차이가 커지고 있다." /></label><label className="flex items-start gap-2 rounded-xl bg-slate-50 p-3"><input className="mt-1" type="checkbox" checked={participant.roleAccepted} onChange={(event) => setParticipant({ ...participant, roleAccepted: event.target.checked })} /><span>여러분은 오늘 C1바이오 영업팀장 이대호 팀장 역할로 판단하고, AI 결과는 답이 아니라 초안으로 다루겠습니다.</span></label></ShellCard></div>;
}

function RoleTeamIntroStep() {
  return <div className="space-y-4"><V40VNextFlowStrip currentStep={2} /><V39StepHero eyebrow="2단계 · 역할과 팀원 구성 이해하기" icon="👥" title="여러분의 역할은 영업팀장 이대호 팀장입니다" tone="indigo" description="이대호 팀장은 C1바이오 영업팀을 맡은 현장형 팀장입니다. 여러분은 이대호 팀장의 입장에서 시장 변화, 전사전략, 팀원 실행 신호를 읽고 성과관리·업무관리·사람관리를 차례로 판단합니다." badges={[{ label: '역할', value: '이대호 팀장', tone: 'indigo', icon: '👤' }, { label: '팀원', value: '함께 하는 팀원 7명', tone: 'emerald', icon: '👥' }, { label: '판단 흐름', value: '성과·업무·사람관리', tone: 'amber', icon: '🧭' }]} /><section className="grid gap-3 md:grid-cols-3"><div className="rounded-3xl border border-cyan-100 bg-cyan-50 p-4"><p className="text-sm font-black text-cyan-950">이대호 팀장 소개</p><p className="mt-2 text-sm leading-6 text-cyan-900">영업 현장의 실행 압박과 팀원 성장 책임을 동시에 안고 있는 팀장입니다. 숫자만 보지 않고, 고객 접점 이후의 행동과 팀원별 실행 신호를 연결해야 합니다.</p></div><div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4"><p className="text-sm font-black text-emerald-950">오늘의 관리 흐름</p><p className="mt-2 text-sm leading-6 text-emerald-900">성과관리에서 기준을 세우고, 업무관리에서 실행 과제로 바꾸며, 사람관리에서 1on1 대화와 행동 합의를 준비합니다.</p></div><div className="rounded-3xl border border-amber-100 bg-amber-50 p-4"><p className="text-sm font-black text-amber-950">판단 원칙</p><p className="mt-2 text-sm leading-6 text-amber-900">AI는 답을 대신 정하지 않습니다. 이대호 팀장인 여러분이 먼저 판단하고, AI 초안은 비교·수정·보완하는 재료로만 사용합니다.</p></div></section><ShellCard title="함께 하는 우리 팀원"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{TEAM_MEMBERS.map((member) => <div key={member.name} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="font-black text-slate-950">{member.name}</p><p className="mt-1 text-xs font-bold text-slate-500">{member.role}</p><p className="mt-2 text-sm leading-6 text-slate-700">{member.signal}</p></div>)}</div><p className="text-xs font-bold leading-5 text-slate-500">이 인물들은 교육용 가상 팀원입니다. 이후 단계에서 여러분은 이대호 팀장으로서 이들의 실행 신호, 업무 흐름, 1on1 대화 초점을 판단합니다.</p></ShellCard></div>;
}
...
function V40TeamMemberStep() { return <div className="space-y-4"><V40VNextFlowStrip currentStep={10} /><V39StepHero eyebrow="사람관리 1 · 먼저 이야기할 팀원 고르기" icon="👤" title="신호를 보고, 해석을 늦추고, 먼저 대화할 팀원을 고릅니다" tone="indigo" description="업무관리 실행 결과를 바탕으로 1on1 대화 초점을 하나로 좁힙니다." badges={[{ label: '인물', value: '팀원 7명', tone: 'indigo', icon: '👥' }, { label: '핵심', value: '관찰과 해석 분리', tone: 'amber', icon: '🔎' }, { label: '다음', value: '1on1 실천', tone: 'emerald', icon: '💬' }]} /><V40VNextPeopleSelectionLab /></div>; }
...
function V40VNextPreviewApp() {
  ...
  void handleReset;
  return <V40VNextStepNavigationProvider onStepSelect={(stepNumber) => goToStep(stepNumber - 1)}><JourneyShell title="C1바이오 영업팀장 AI 리더십 Lab Journey" steps={V40_VNEXT_VISIBLE_APP_STEPS} currentStep={safeStep} onPrev={() => goToStep(safeStep - 1)} onNext={() => goToStep(safeStep + 1)} onStepSelect={goToStep} hideStepOverview={true}><V40VNextProgressCoachPanel currentStep={safeStep} participant={participant} onStepSelect={goToStep} />{renderStep(safeStep, participant, setParticipant)}</JourneyShell></V40VNextStepNavigationProvider>;
}
