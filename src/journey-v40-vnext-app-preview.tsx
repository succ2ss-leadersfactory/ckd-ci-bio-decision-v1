...
const V40_VNEXT_STORAGE_KEYS = { participant: 'ckd.v40-vnext.participant.v1', progress: 'ckd.v40-vnext.progress.v1' };
const DEFAULT_PARTICIPANT: V40VNextParticipant = { groupName: '', tableName: '', representativeSituation: '', roleAccepted: false };
...
function scrollV40VNextToTop() { window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })); }
function V40ComplianceNotice() { return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><p className="font-black">AI에 그대로 넣으면 안 되는 말</p><p className="mt-1">실제 고객명, 병원명, 의료진명, 제품명, 실제 매출자료, 처방 정보, 팀원 실명, 내부 전략, 개인정보, 개인 성과등급, 미승인 효능, 허가 외 사용 암시, 처방 유도 표현, 경쟁사 비방, 비교 우위 단정은 입력하지 않습니다.</p></div>; }
function ShellCard({ title, children }: { title: string; children: ReactNode }) { return <section className="rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-lg font-black text-slate-900">{title}</h3><div className="mt-3 space-y-3 text-sm leading-6 text-slate-700">{children}</div></section>; }
...
function EntryStep({ participant, setParticipant }: { participant: V40VNextParticipant; setParticipant: (next: V40VNextParticipant) => void }) {
  return <div className="space-y-4"><V40VNextFlowStrip currentStep={1} /><V39StepHero eyebrow="1단계 · 조별 역할 잡기" icon="🤝" title="오늘은 우리 조가 C1바이오 영업팀장입니다" tone="indigo" description="개인 답을 쓰는 시간이 아닙니다. 우리 조가 하나의 대표 상황을 정하고, 시장 변화와 고객 기록, 팀원 움직임을 놓고 다음 2주 실행 메모를 함께 만듭니다. 모든 문장은 조별 실습형으로 남깁니다." badges={[{ label: '운영 방식', value: '조별 실습', tone: 'indigo', icon: '🤝' }, { label: '조 구성', value: '2~3명', tone: 'emerald', icon: '👥' }, { label: '핵심 산출물', value: '2주 실행 메모', tone: 'amber', icon: '📝' }]} /><section className="rounded-3xl border border-indigo-100 bg-white p-4 shadow-sm md:p-5"><V39MiniFlow items={[{ icon: '🤝', title: '우리 조 역할 잡기', body: 'C1바이오 영업팀장 관점에서 판단합니다.' }, { icon: '🛡️', title: '말해도 되는 선 확인', body: '실제 민감정보는 모두 빼고 가상 상황으로 씁니다.' }, { icon: '📝', title: '대표 상황 정하기', body: '우리 조가 다룰 대표 상황을 1개로 좁힙니다.' }]} /><div className="mt-3"><V39MinimumChecklist tone="indigo" items={['조/팀 선택', '대표 상황 1개 작성', '조별 실습 역할 확인']} /></div></section><V40ComplianceNotice /><ShellCard title="우리 조가 다룰 대표 상황"><div className="grid gap-3 md:grid-cols-2"><label className="space-y-1"><span className="text-xs font-bold text-slate-500">조 이름</span><select className="w-full rounded-xl border bg-white px-3 py-2" value={participant.groupName} onChange={(event) => setParticipant({ ...participant, groupName: event.target.value })}><option value="">조를 선택하세요</option>{GROUP_OPTIONS.map((group) => <option key={group} value={group}>{group}</option>)}</select></label><label className="space-y-1"><span className="text-xs font-bold text-slate-500">운영 팀/테이블</span><select className="w-full rounded-xl border bg-white px-3 py-2" value={participant.tableName} onChange={(event) => setParticipant({ ...participant, tableName: event.target.value })}><option value="">테이블을 선택하세요</option>{TABLE_OPTIONS.map((team) => <option key={team} value={team}>{team}</option>)}</select></label></div><label className="block space-y-1"><span className="text-xs font-bold text-slate-500">우리 조가 다룰 대표 상황</span><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={participant.representativeSituation} onChange={(event) => setParticipant({ ...participant, representativeSituation: event.target.value })} placeholder="예: 활동 기록은 늘었지만 고객 반응 이후 후속 실행이 약하고, 팀원별 기록 품질 차이가 커지고 있다." /></label><label className="flex items-start gap-2 rounded-xl bg-slate-50 p-3"><input className="mt-1" type="checkbox" checked={participant.roleAccepted} onChange={(event) => setParticipant({ ...participant, roleAccepted: event.target.checked })} /><span>우리 조는 오늘 C1바이오 영업팀장 역할로 판단하고, AI 결과는 답이 아니라 초안으로 다루겠습니다.</span></label></ShellCard></div>;
}

function V40AiSafetyStep() { return <div className="space-y-4"><V40VNextFlowStrip currentStep={2} /><V39StepHero eyebrow="2단계 · 말해도 되는 선 확인" icon="🛡️" title="조별 토의 내용도 AI에 넣기 전에는 한 번 지워야 합니다" tone="amber" description="우리 조가 이야기한 현장 사례를 그대로 넣지 않습니다. 실제 고객, 기관, 제품, 수치, 사람을 가릴수록 AI는 더 안전한 초안 도구가 됩니다." badges={[{ label: '먼저 할 일', value: '민감정보 제거', tone: 'amber', icon: '🛡️' }, { label: '방식', value: '가상·익명화', tone: 'slate', icon: '✂️' }, { label: '다음', value: '질문 다듬기', tone: 'violet', icon: '✍️' }]} /><V39SafetyStrip>고객을 평가하거나 등급화하지 않습니다. 기록에서 다음 행동의 단서를 찾고, AI 초안은 팀장 언어로 다시 고칩니다.</V39SafetyStrip><AiSafetyLab /></div>; }
...
function V40VNextPreviewApp() {
  ...
  return <V40VNextStepNavigationProvider onStepSelect={(stepNumber) => goToStep(stepNumber - 1)}><JourneyShell title="C1바이오 영업팀장 AI 리더십 Lab Journey v40-vNext" subtitle="v39의 산출물, UI/UX, 현장언어, 등장인물을 계승하고 성과관리 → 업무관리 → 사람관리 순서로 재구성한 조별 실습형 후속 버전입니다." steps={V40_VNEXT_VISIBLE_APP_STEPS} currentStep={safeStep} onPrev={() => goToStep(safeStep - 1)} onNext={() => goToStep(safeStep + 1)} onStepSelect={goToStep} hideStepOverview={false}><div className="mb-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:p-5"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-cyan-700">v40-vNext 보호 기준</p><p className="mt-1 text-sm font-bold leading-6 text-slate-600">기존 v39는 기준 원본으로 보호합니다. 이 화면은 별도 route에서만 작동하며, 성과관리는 4·5단계로 압축하고 6·7단계는 숨긴 기본 운영 흐름으로 진행합니다.</p></div><button type="button" className="rounded-2xl border border-slate-300 px-4 py-3 text-sm font-black text-slate-700" onClick={handleReset}>v40-vNext 입력 초기화</button></div></div><V40VNextProgressCoachPanel currentStep={safeStep} participant={participant} onStepSelect={goToStep} />{renderStep(safeStep, participant, setParticipant)}</JourneyShell></V40VNextStepNavigationProvider>;
}
