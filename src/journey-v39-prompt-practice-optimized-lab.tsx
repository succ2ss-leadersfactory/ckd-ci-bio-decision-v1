...
const CONCERN_OPTIONS: ConcernOption[] = [
  {
    ...
    situationSummary: '방문 횟수는 유지되고 있지만 고객 질문, 자료 요청, 다음 약속, 후속 확인으로 이어지는 경우가 적습니다.',
  ...
  },
  {
    ...
    situationSummary: '고객 질문이나 자료 요청이 있었지만 이것을 실제 기회 신호로 봐도 되는지 판단이 어렵습니다.',
    ...
    downstreamHint: '고객 Data 확인 List → 고객군별 2주 대응 방향 → 안전한 다음 질문',
  },
  {
    ...
    task: '팀장이 먼저 1on1로 맞춰볼 코칭 대상과 코칭 초점을 판단할 때 확인해야 할 신호를 정리해 주세요. 대화 스크립트는 만들지 말고, 9단계에서 실행 대화로 바꿀 수 있는 판단 기준까지만 정리해 주세요.',
    downstreamHint: '코칭 대상 선정 → 실행 대화 → 2주 실행계획',
  },
  {
    id: 'ai-boundary',
...
function buildFinalPrompt(response: PromptPracticeResponse) {
  return [
    ...
    '',
    '[요청]',
    response.task || '팀장이 확인해야 할 원인 가설과 이번 2주 동안 볼 관리 기준을 정리해 주세요.',
    '',
    '[출력 형식]',
...
function OptionButton({ option, selected, onClick }: { option: ConcernOption; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-2xl border p-4 text-left shadow-sm ${selected ? 'border-violet-300 bg-violet-50 text-violet-950' : 'bg-white text-slate-700'}`}>
      ...
      <p className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-[11px] font-bold leading-5 text-slate-600">{option.downstreamHint}</p>
    </button>
  );
}
  ...
  return (
    <section className="space-y-4">
      <section className="rounded-3xl border border-violet-100 bg-white p-5 shadow-sm md:p-6">
        <p className="text-xs font-black uppercase tracking-wide text-violet-700">Block 1</p>
        <h3 className="text-xl font-black text-slate-950">우리 팀 고민을 AI가 알아듣는 질문으로 바꾸기</h3>
        <p className="mt-2 text-sm font-bold leading-6 text-slate-600">AI에게 바로 답을 묻기 전에, 우리 팀 고민을 역할·맥락·요청·출력 형식으로 나눕니다. 이 단계에서 고른 고민은 4단계 전략 리서치, 5단계 관리 지표, 6단계 고객 Data 확인 List, 7단계 고객군별 2주 대응 방향, 8단계 코칭 대상 선정으로 이어집니다.</p>
        ...
        <label className="mt-4 block space-y-1">
          <span className="text-xs font-black text-slate-500">직접 입력할 고민</span>
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-bold leading-6 text-emerald-950">
            <p className="font-black">구조화 질문</p>
            <p className="mt-2">역할, 상황, 요청, 출력 형식을 함께 제시해 AI가 현장 맥락에 맞게 정리하도록 요청합니다.</p>
          </div>
        </div>
      </section>

      <section className="rounded-3xl border border-sky-100 bg-white p-5 shadow-sm md:p-6">
        <p className="text-xs font-black uppercase tracking-wide text-sky-700">Block 3</p>
        <h3 className="text-xl font-black text-slate-950">역할·맥락·요청·출력 형식</h3>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <label className="space-y-1"><span className="text-xs font-black text-slate-500">역할</span><textarea className="min-h-24 w-full rounded-2xl border px-3 py-2 text-sm leading-6" value={response.roleText} onChange={(event) => persist({ roleText: event.target.value })} /></label>
          <label className="space-y-1"><span className="text-xs font-black text-slate-500">상황 설명</span><textarea className="min-h-24 w-full rounded-2xl border px-3 py-2 text-sm leading-6" value={response.context} onChange={(event) => persist({ context: event.target.value })} /></label>
          <label className="space-y-1"><span className="text-xs font-black text-slate-500">요청</span><textarea className="min-h-24 w-full rounded-2xl border px-3 py-2 text-sm leading-6" value={response.task} onChange={(event) => persist({ task: event.target.value })} /></label>
          <label className="space-y-1"><span className="text-xs font-black text-slate-500">출력 형식</span><textarea className="min-h-24 w-full rounded-2xl border px-3 py-2 text-sm leading-6" value={response.format} onChange={(event) => persist({ format: event.target.value })} /></label>
        </div>
      </section>

      <section className="rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm md:p-6">
        <p className="text-xs font-black uppercase tracking-wide text-emerald-700">Block 4</p>
        <h3 className="text-xl font-black text-slate-950">4단계 AI 전략 리서치로 넘길 질문</h3>
        <p className="mt-2 text-sm font-bold leading-6 text-slate-600">아래 질문을 복사해 AI 도구에 붙여넣습니다. AI 결과는 그대로 확정하지 않고, 다음 단계에서 관리 지표와 고객 Data 확인 List로 다시 바꿉니다.</p>
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-black text-white" onClick={copyPrompt}>{copied ? '복사 완료' : '프롬프트 복사'}</button></div>
      ...
      <section className="rounded-3xl border border-amber-100 bg-amber-50 p-4 text-xs font-bold leading-5 text-amber-950">
        <p className="font-black">입력 안전선</p>
