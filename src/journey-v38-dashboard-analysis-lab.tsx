    ...
    <div className="mt-4">
      ...
      <div className="mt-2 grid gap-2 md:grid-cols-2">
        {options.map((metric) => {
          ...
          return (
            <label key={metric.id} className={`rounded-2xl border p-3 text-xs font-bold leading-5 ${checked ? 'border-cyan-700 bg-cyan-50 text-cyan-950' : 'bg-white text-slate-700'}`}>
              <div className="flex gap-2">
                <input type="checkbox" checked={checked} onChange={() => setter((current) => current.includes(metric.id) ? current.filter((item) => item !== metric.id) : current.length >= max ? current : [...current, metric.id])} />
...
function ReviewTextarea({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="space-y-1">
      ...
      <textarea className="min-h-24 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={value} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
...
function PrepTextarea({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="space-y-1">
      <span className="text-xs font-black text-slate-500">{label}</span>
      <textarea className="min-h-24 w-full rounded-2xl border bg-white px-3 py-2 text-sm leading-6" value={value} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
