...
function getMetric(id: string) { return METRIC_OPTIONS.find((item) => item.id === id); }
function getSuggestedDeliverables(memberId: string) {
  const map: Record<string, string[]> = { M01: ['후속 대화 연결 코칭 질문', '방문 후 기록 점검표'], M02: ['강점 활용 역할 제안', '팀 회의 공유용 학습 포인트'], M03: ['고객 질문 연습 스크립트', '1on1 면담 질문'], M04: ['컴플라이언스 안전선 점검 문장', '우려 신호 확인 질문'], M05: ['통제 가능한 실행 변수 찾기 질문', '작은 실행 약속 카드'], M06: ['실행 점검 기준', '방문 후 기록 점검표'] };
  return map[memberId] ?? [];
}
...
export function V38DashboardAnalysisLab() {
  ...
  const metricPrompt = useMemo(() => [
    ...
    '우리 팀 상황:',
    ...(teamSituations.length > 0 ? teamSituations.map((item) => `- ${item}`) : ['- 아직 선택하지 않았습니다.']),
    '요청: 우리 팀 상황에 맞는 안전한 실행 관찰 지표 후보를 제안해 주세요.',
    '출력 형식: 1. 지표 영역 2. 지표 이름 3. 이 지표를 봐야 하는 이유 4. 이 지표만 보고 단정하면 안 되는 점 5. 함께 봐야 할 보완 지표 6. 팀장이 던질 수 있는 확인 질문',
    '주의:',
  ...
  ].join('\n'), [selectedMetricSummary, metricRationale]);
  ...
  const prepPrompt = useMemo(() => [
    ...
    '팀원별 분리 정리와 팀장 행동 선택:',
    ...TEAM_MEMBERS.flatMap((member, index) => { const current = memberPreps[member.id] ?? emptyPrep(); const choices = selectedDeliverables[member.id] ?? []; return [`${index + 1}. ${member.name}`, `- 팀원별 관찰 신호: ${current.observedSignal || '미작성'}`, `- 강점으로 볼 수 있는 신호: ${current.strengthSignal || '미작성'}`, `- 우려 또는 확인이 필요한 신호: ${current.concernSignal || '미작성'}`, `- 추가로 확인해야 할 질문: ${current.checkQuestion || '미작성'}`, `- 성급하게 단정하면 안 되는 점: ${current.doNotAssume || '미작성'}`, `- 팀장 행동 선택: ${choices.length > 0 ? choices.join(' / ') : '미선택'}`, '']; }),
    '선택 가능한 준비물:',
  ...
  const toggleString = (value: string, setter: React.Dispatch<React.SetStateAction<string[]>>) => setter((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value]);
  const toggleMetric = (id: string, setter: React.Dispatch<React.SetStateAction<string[]>>, max: number) => setter((current) => current.includes(id) ? current.filter((item) => item !== id) : current.length >= max ? current : [...current, id]);
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-3xl border bg-white p-5 shadow-sm md:p-6"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-cyan-700">v38 Dashboard Analysis Lab</p><h2 className="mt-2 text-2xl font-black text-slate-950">우리 팀 지표로 다음 행동 준비하기</h2><p className="mt-2 text-sm leading-6 text-slate-600">이 단계는 <span className="font-black text-slate-950">우리 팀 지표 정하기 → 6명 팀원 Data 전체 보기 → AI로 신호 뽑기 → 팀원별로 분리 정리 → 필요한 준비물 만들기</span> 흐름으로, 팀장이 사용할 <span className="font-black text-slate-950">팀원별 다음 행동 준비물</span>을 만듭니다.</p></div><div className="grid gap-2 text-sm font-black text-cyan-800 md:text-right"><div className="rounded-2xl bg-amber-50 px-4 py-3 text-amber-800">지표 선택 {selectedCoreMetrics.length + selectedSupportMetrics.length + selectedSafetyMetrics.length} / 6</div><div className="rounded-2xl bg-cyan-50 px-4 py-3">신호 분리 {completedSignalCount} / {TEAM_MEMBERS.length}</div><div className="rounded-2xl bg-indigo-50 px-4 py-3 text-indigo-800">행동 선택 {completedActionChoiceCount} / {TEAM_MEMBERS.length}</div><div className="rounded-2xl bg-emerald-50 px-4 py-3 text-emerald-800">준비물 완성 {completedFinalCount} / {TEAM_MEMBERS.length}</div></div></div></div>

      <div className="rounded-3xl border border-amber-100 bg-amber-50 p-5 shadow-sm md:p-6"><p className="text-xs font-black uppercase tracking-wide text-amber-700">Block 0</p><h3 className="text-lg font-black text-slate-950">우리 팀 지표 정하기</h3><p className="mt-2 rounded-2xl bg-white p-3 text-xs font-bold leading-5 text-slate-600">이 지표는 평가 점수표가 아닙니다. 팀장이 무엇을 확인하고 어떤 대화를 준비할지 정하기 위한 <span className="font-black text-slate-950">코칭 신호</span>입니다.</p><div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-4"><h4 className="text-sm font-black text-slate-950">우리 팀 상황 선택</h4><div className="mt-3 grid gap-2">{TEAM_SITUATION_OPTIONS.map((option) => <label key={option} className={`flex gap-3 rounded-2xl border p-3 text-xs font-bold leading-5 ${teamSituations.includes(option) ? 'border-amber-700 bg-amber-50 text-amber-950' : 'bg-white text-slate-700'}`}><input type="checkbox" checked={teamSituations.includes(option)} onChange={() => toggleString(option, setTeamSituations)} /><span>{option}</span></label>)}</div></div><div className="rounded-2xl border bg-white p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h4 className="text-sm font-black text-slate-950">AI 지표 추천 프롬프트</h4><p className="mt-1 text-xs font-bold leading-5 text-slate-600">우리 팀 상황을 기준으로 안전한 실행 관찰 지표 후보를 받아봅니다.</p></div><button type="button" className="rounded-2xl bg-amber-700 px-4 py-3 text-sm font-black text-white" onClick={() => copyPrompt('metric')}>{copiedPrompt === 'metric' ? '복사 완료' : 'AI 지표 추천 프롬프트 복사'}</button></div><pre className="mt-4 max-h-72 overflow-auto whitespace-pre-wrap rounded-2xl bg-slate-950 p-4 text-xs leading-5 text-slate-100">{metricPrompt}</pre></div></div><label className="mt-4 block space-y-1"><span className="text-xs font-black text-slate-600">AI 추천 지표 붙여넣기</span><textarea className="min-h-28 w-full rounded-2xl border bg-white px-3 py-2 text-sm leading-6" value={aiMetricSuggestion} onChange={(event) => setAiMetricSuggestion(event.target.value)} placeholder="AI가 추천한 지표 후보를 붙여넣으세요. 그대로 확정하지 않고 아래에서 우리 팀 기준으로 선택합니다." /></label><div className="mt-4 rounded-2xl border bg-white p-4"><h4 className="text-sm font-black text-slate-950">우리 팀 핵심 실행지표 선택</h4><p className="mt-1 text-xs font-bold leading-5 text-slate-600">핵심 지표 3개, 보완 지표 2개, 안전선 지표 1개를 선택하세요. 안전선 지표는 최소 1개가 필요합니다.</p><MetricPicker title="핵심 지표 3개" selected={selectedCoreMetrics} setter={setSelectedCoreMetrics} max={3} safetyOnly={false} /><MetricPicker title="보완 지표 2개" selected={selectedSupportMetrics} setter={setSelectedSupportMetrics} max={2} safetyOnly={false} /><MetricPicker title="안전선 지표 1개" selected={selectedSafetyMetrics} setter={setSelectedSafetyMetrics} max={1} safetyOnly={true} /><label className="mt-4 block space-y-1"><span className="text-xs font-black text-slate-600">지표 선택 이유</span><textarea className="min-h-20 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={metricRationale} onChange={(event) => setMetricRationale(event.target.value)} placeholder="예: 우리 팀은 활동량보다 고객 반응과 후속 대화 연결을 우선 확인해야 한다." /></label></div></div>
