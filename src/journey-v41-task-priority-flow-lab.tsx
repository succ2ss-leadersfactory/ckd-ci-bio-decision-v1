...
function normalizeCriterionLine(line: string, item?: string) {
  ...
  return text || '해당 업무의 완료 여부를 확인할 수 있음';
}
...
function criterionForWorkItem(item: string, criteria: string[], index: number) {
  ...
  return normalizeCriterionLine(criteria[index] || '해당 업무의 완료 여부를 확인할 수 있음', item);
}
...
function cleanOutputName(output?: string, managementTask?: string, items: string[] = []) {
  const raw = compact(output);
  const basis = `${raw} ${compact(managementTask)} ${items.join(' ')}`;
  ...
  if (raw === '미작성') return '업무 실행 현황표';
  return raw
    .replace(/의 결과로 남길 업무산출물/g, '')
    .replace(/산출물$/g, '현황표')
    .replace(/\s+/g, ' ')
    .trim();
}

function cleanLocationForDisplay(location?: string) {
  const raw = compact(location);
  if (raw === '미작성') return '팀장이 정한 기록 위치 1곳';
  if (raw.includes('중 하나') || raw.includes(',')) return '팀장이 정한 기록 위치 1곳';
  return raw;
}

function cleanLocationForPrompt(location?: string) {
  const raw = compact(location);
  const display = cleanLocationForDisplay(location);
  if (raw === display) return display;
  return `${display}\n- 원문 후보: ${raw}\n- 주의: AI가 기록 위치를 임의로 고르지 말고, “팀장이 정한 기록 위치 1곳”으로만 표현하세요.`;
}

function cleanCompletionStandard(completion?: string, managementTask?: string) {
  const raw = compact(completion);
  const task = compact(managementTask);
  if (raw === '미작성') return '산출물 등록 여부, 누락 항목, 다음 행동 입력 여부를 확인할 수 있어야 한다.';
  if (task !== '미작성' && raw.includes(task)) return '산출물 등록 여부, 누락 항목, 다음 행동 입력 여부를 확인할 수 있어야 한다.';
  return short(raw, 140);
}

function cleanEvidence(selectedEvidence: string[]) {
  const checks = selectedEvidence.length ? selectedEvidence : EVIDENCE_CHECKS.slice(0, 3);
  return checks.join(', ');
}

function finalCheckTiming(cycle: string, checkpointTitle: string) {
  if (cycle === '1주' && checkpointTitle === '1주차 종료') return '중간 확인: 마감 전 2일 / 최종 확인: 1주차 종료';
...
function buildSequenceDraft(items: string[], criteria: string[], optionId?: string) {
  ...
  return [
    ...
    '',
    ...source.map((item, index) => `${index + 1}. ${item}\n- 시작 조건: 이전 단계 입력값과 기존 기록을 확인할 수 있음\n- 완료 기준: ${criterionForWorkItem(item, criteria, index)}`),
  ].join('\n');
}
...
function buildCheckpointDraft(params: { cycle: string; output: string; location: string; completion: string; checkpointId?: string; selectedQuestions?: string[]; selectedEvidence?: string[] }) {
  ...
  const evidence = cleanEvidence(params.selectedEvidence ?? []);
  return [
...
function buildWorkloadDraft(reduceTasks: string[], reasons: string[]) {
  ...
  return ['이번 주기에 잠시 줄일 일:', ...selected.map((task) => `- ${task}`), '', '선택 이유:', ...reasonText.map((reason) => `- ${reason}`)].join('\n');
}

function buildInstructionFromFields(state: TaskState, output: string, location: string, completion: string) {
...
export function V41TaskPriorityFlowLab() {
  ...
  const outputForInstruction = cleanOutputName(state.selectedOutput, state.managementTask, workItems);
  const locationDisplay = cleanLocationForDisplay(state.outputLocation);
  const locationPrompt = cleanLocationForPrompt(state.outputLocation);
  const completionDisplay = cleanCompletionStandard(state.completionStandard, state.managementTask);
  const evidenceDisplay = cleanEvidence(selectedEvidence);
  ...
  const checkpointDraft = buildCheckpointDraft({ cycle: executionCycle, output: outputForInstruction, location: locationDisplay, completion: completionDisplay, checkpointId: state.checkpointDecisionType, selectedQuestions, selectedEvidence });
  ...
  const applyAiDecisionRecommendation = () => update({
    ...
    selectedWorkloadReductionReasons: state.selectedWorkloadReductionReasons?.length ? state.selectedWorkloadReductionReasons : ['이번 단계의 목적은 산출물 완성과 완료 기준 확인에 집중하는 것이기 때문입니다.', '역할과 일정이 정해지기 전에 모든 업무를 동시에 시작하면 중복과 누락이 커질 수 있기 때문입니다.'],
    step7AiPrompt: '',
  });
  ...
  const buildAiPrompt = () => update({
    ...
    step7AiPrompt: [
      ...
      '',
      '[작성 원칙]',
      '1. 아래 정제된 선택값만 사용해 업무지시 초안을 작성합니다.',
      '2. 원문 표현을 반복하지 말고, 팀원이 실행할 수 있는 업무지시 문장으로 바꿉니다.',
      '3. 산출물명은 아래 명사형 산출물명만 사용합니다.',
  ...
  const makeInstructionDraft = () => {
    const baseDraft = compact(state.step7AiResult) !== '미작성'
      ? state.step7AiResult
      : buildInstructionFromFields({ ...state, orderedWorkSteps: state.orderedWorkSteps || sequenceDraft, roleResponsibilityMap: state.roleResponsibilityMap || roleDraft, scheduleCheckpoints: state.scheduleCheckpoints || checkpointDraft, workloadAdjustments: state.workloadAdjustments || workloadDraft }, outputForInstruction, locationDisplay, completionDisplay);
  ...
  const makeStep8Handoff = () => update({
    step7HandoffToStep8: ['[8단계 전달 메모]', `선택한 경계·병목 후보: ${selectedStep8Signals.length ? selectedStep8Signals.join(', ') : '업무 경계 필요, 병목 예상'}`, `업무 경계가 필요한 부분: ${excluded.length ? excluded.join(', ') : '6단계에서 제외한 업무와 최종 선택 업무 단위의 경계'}`, `${checkpointOption.title}까지 확인 증거가 남지 않는 경우 병목 후보로 봅니다.`, '담당자 권한 밖 이슈: 고객 데이터 접근, 부서 협조, 시스템 입력 권한, 기록 위치 변경 등', '에스컬레이션 후보: 반복 지연, 산출물 미등록, 필수 기록 누락, 역할 충돌', '주의: 이 메모는 사람 판단이 아니라 업무 경계와 프로세스 병목 후보입니다.'].join('\n'),
  });
  ...
  return <section className="space-y-4">
    ...
    <Card title="1. 6단계 실행계획 확인" tone="amber">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-600">
        <p className="mb-2 text-xs font-black text-amber-700">6단계 요약 입력값 · 업무지시용 정제값</p>
        ...
        <SummaryLine label="기록 위치" value={locationDisplay} />
        <SummaryLine label="완료 기준" value={completionDisplay} />
        <SummaryLine label="확인 증거" value={evidenceDisplay} />
      </div>
    ...
    <Card title="2. 실행 순서 결정하기" tone="cyan">
      ...
      <div className="grid gap-3 md:grid-cols-2">{SEQUENCE_OPTIONS.map((option) => <ChoiceCard key={option.id} selected={sequenceOption.id === option.id} title={option.title} description={option.description} onClick={() => update({ selectedSequenceOption: option.id, sequenceDecisionType: '팀장 선택', step7AiPrompt: '' })} />)}</div>
    ...
    <Card title="3. 역할과 책임 결정하기" tone="cyan">
      ...
      <div className="space-y-3">{(workItems.length ? workItems : ['최종 선택 업무 단위']).map((item, index) => { const role = { ...defaultRoleDecision(), ...(roleMatrix[item] ?? {}) }; return <div key={`${item}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-black text-slate-950">{index + 1}. {item}</p><div className="mt-3 grid gap-3 md:grid-cols-3"><label className="text-xs font-black text-slate-600">주 실행<SelectBox value={role.owner ?? OWNER_OPTIONS[0]} options={OWNER_OPTIONS} onChange={(value) => updateRoleDecision(item, { owner: value })} /></label><label className="text-xs font-black text-slate-600">팀장 확인<SelectBox value={role.leaderCheck ?? LEADER_CHECK_OPTIONS[0]} options={LEADER_CHECK_OPTIONS} onChange={(value) => updateRoleDecision(item, { leaderCheck: value })} /></label><label className="text-xs font-black text-slate-600">협업 방식<SelectBox value={role.collaboration ?? COLLABORATION_OPTIONS[0]} options={COLLABORATION_OPTIONS} onChange={(value) => updateRoleDecision(item, { collaboration: value })} /></label></div></div>; })}</div>
    ...
    <Card title="4. 일정과 점검방식 결정하기" tone="cyan">
      <div className="grid gap-3 md:grid-cols-2">{CHECKPOINT_OPTIONS.map((option) => <ChoiceCard key={option.id} selected={checkpointOption.id === option.id} title={option.title} description={option.description} onClick={() => update({ checkpointDecisionType: option.id, step7AiPrompt: '' })} />)}</div>
      <div className="grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-sm font-black text-slate-950">확인할 증거 선택</p><div className="mt-3 grid gap-2">{EVIDENCE_CHECKS.map((item) => <MultiCheck key={item} checked={selectedEvidence.includes(item)} label={item} onChange={() => update({ selectedEvidenceChecks: toggle(selectedEvidence, item, 4), step7AiPrompt: '' })} />)}</div></div><div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-sm font-black text-slate-950">중간 확인 질문 선택</p><div className="mt-3 grid gap-2">{CHECKPOINT_QUESTIONS.map((item) => <MultiCheck key={item} checked={selectedQuestions.includes(item)} label={item} onChange={() => update({ selectedCheckpointQuestions: toggle(selectedQuestions, item, 3), step7AiPrompt: '' })} />)}</div></div></div>
    ...
    <Card title="5. 업무량 조정: 이번 주기에는 잠시 줄일 일" tone="amber">
      ...
      <div className="grid gap-2 md:grid-cols-2">{REDUCE_TASKS.map((task) => <MultiCheck key={task} checked={reduceTasks.includes(task)} label={task} onChange={() => update({ selectedReduceTasks: toggle(reduceTasks, task, 3), step7AiPrompt: '' })} />)}</div>
    ...
    <Card title="6. AI로 업무지시 초안 만들기" tone="violet">
      <div className="grid gap-3 md:grid-cols-4">{INSTRUCTION_TONES.map((tone) => <ChoiceCard key={tone} selected={(state.step7InstructionTone || INSTRUCTION_TONES[0]) === tone} title={tone} description="AI 초안의 표현 방식을 선택합니다." onClick={() => update({ step7InstructionTone: tone, step7AiPrompt: '' })} />)}</div>
    ...
    <Card title="7. 사람 검토 후 최종 업무지시 확정" tone="emerald">
      <div className="grid gap-2 md:grid-cols-3">{REVIEW_CHECKLIST.map((item) => <MultiCheck key={item} checked={reviewChecklist.includes(item)} label={item} onChange={() => update({ step7ReviewChecklist: toggle(reviewChecklist, item, REVIEW_CHECKLIST.length) })} />)}</div>
    ...
    <Card title="8. 8단계 업무 경계·병목 대응으로 넘기기" tone="slate">
      ...
      <div className="grid gap-2 md:grid-cols-3">{STEP8_HANDOFF_SIGNALS.map((signal) => <MultiCheck key={signal} checked={selectedStep8Signals.includes(signal)} label={signal} onChange={() => update({ selectedStep8HandoffSignals: toggle(selectedStep8Signals, signal, STEP8_HANDOFF_SIGNALS.length) })} />)}</div>
