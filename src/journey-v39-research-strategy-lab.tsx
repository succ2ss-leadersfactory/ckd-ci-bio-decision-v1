    ...
    <div className="space-y-4">
      ...
      <SectionCard title="1단계: 리서치 주제 선택과 Perplexity 탐색">
        ...
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3 text-xs font-bold leading-5 text-emerald-950">
          <p className="font-black">이 주제는 다음 관리 지표로 연결될 수 있습니다.</p>
          <p className="mt-1">{bridgeHints.join(' · ')}</p>
        </div>
        <label className="block space-y-1">
      ...
      <SectionCard title="5단계 연결 카드: 관리 지표로 바꿀 실행 질문">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold leading-6 text-emerald-950">
          <p className="font-black">이 카드가 5단계 입력값입니다.</p>
          <p className="mt-1">전략 이슈를 회의자료로 끝내지 않고, 다음 단계에서 우리 팀의 2주 관리 지표로 바꾸기 위한 질문을 남깁니다.</p>
        </div>
        ...
        <div className="rounded-2xl bg-slate-50 p-3 text-xs font-bold leading-5 text-slate-600">
          <p className="font-black text-slate-900">현재 주제에서 관리 지표로 연결 가능한 후보</p>
          <p className="mt-1">{bridgeHints.join(' · ')}</p>
        </div>
      </SectionCard>

      <SectionCard title="6단계: NotebookLM Studio 보고서 초안 생성">
      ...
      <SectionCard title="7단계: NotebookLM Studio 발표용 10장 슬라이드 구성">
      ...
      <SectionCard title="8단계: NotebookLM Studio 인포그래픽 초안 생성">
        ...
        <button type="button" className="rounded-xl bg-cyan-700 px-4 py-2 text-sm font-black text-white" onClick={() => copyText(infographicPrompt, '인포그래픽 요청문')}>인포그래픽 요청문 복사</button>
      ...
      <SectionCard title="9단계: 전략회의 발표 준비와 최종 점검">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {REVIEW_ITEMS.map((item) => <label key={item} className="flex items-start gap-2 rounded-xl border p-3 text-sm font-bold leading-5"><input type="checkbox" className="mt-1" checked={Boolean(response.reviewChecks?.[item])} onChange={(event) => update({ reviewChecks: { ...response.reviewChecks, [item]: event.checked } })} /><span>{item}</span></label>)}
        </div>
