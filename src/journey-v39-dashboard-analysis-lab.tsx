  ...
  return (
    <article className={`rounded-3xl border p-5 ${selected ? 'border-cyan-500 bg-cyan-50' : disabled ? 'bg-slate-100 opacity-60' : 'bg-white'}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" disabled={disabled} className={`shrink-0 rounded-2xl px-3 py-2 text-xs font-black ${selected ? 'bg-cyan-700 text-white' : 'border bg-white text-slate-700'}`} onClick={onToggle}>
      ...
      <div className="mt-4 space-y-3 text-sm font-bold leading-6 text-slate-700">
        ...
        <p><span className="font-black text-slate-950">이번 지표와의 연결: </span>선택한 핵심 실행 지표가 흔들릴 때, 이 유형의 행동 신호를 함께 살펴봅니다.</p>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        {member.signals.slice(0, 4).map((signal) => (
          <span key={signal} className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600 shadow-sm">{toSignalWords(signal)}</span>
        ))}
      </div>
    </article>
  );
...
export function V39DashboardAnalysisLab() {
  ...
  const coreMetricItems = useMemo(
    () => [...selectedCoreMetrics, ...(extraMetric.trim() ? [extraMetric.trim()] : [])],
    [selectedCoreMetrics, extraMetric],
  );
  ...
  useEffect(() => {
    ...
    nextResult.memberResult = {
      ...
      memberSplitNotice: selectedMemberIds.length > 0 ? '팀 현황에서 팀원 실행진단으로 연결됨' : '',
      memberPreps,
  ...
  const toggleMetric = (label: string) => {
    setSelectedCoreMetrics((current) => (current.includes(label) ? current.filter((item) => item !== label) : [...current, label]));
  };
  ...
  const updatePrep = (memberId: string, field: keyof V38MemberPrep, value: string) => {
    setMemberPreps((current) => updateMemberPrep(current, memberId, field, value));
  };
    ...
    <section className="space-y-4">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_21rem] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-4xl text-base font-bold leading-8 text-slate-600">
              먼저 팀 전체 상황을 보고 핵심 실행 지표를 고릅니다. 그다음 이 지표에 영향을 주는 팀원 행동 신호로 자연스럽게 넘어갑니다.
            </p>
          </div>
      ...
      <div className="rounded-3xl border border-amber-100 bg-amber-50 p-5 shadow-sm md:p-6">
        ...
        <p className="mt-2 rounded-2xl bg-white p-3 text-xs font-bold leading-5 text-slate-600">
          우리 팀과 가장 가까운 상황 2개를 고르고, 우리 팀만의 상황을 한 문장으로 더합니다. AI는 각 상황별로 핵심 실행 지표 후보 2개씩을 제안합니다.
        </p>
          ...
          <div className="rounded-2xl border bg-white p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs font-bold leading-5 text-slate-600">상황별 핵심 실행 지표 후보를 뽑는 질문입니다.</p>
              </div>
        ...
        <div className="mt-3">
          <button type="button" className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-black text-white" onClick={splitAiDraft}>AI 초안에서 후보와 참고 신호 나누기</button>
        ...
        <div className="mt-4 rounded-2xl border bg-white p-4">
          ...
          <p className="mt-1 text-xs font-bold leading-5 text-slate-600">AI가 상황별로 제안한 후보 중 이번 2주에 실제로 볼 지표를 선택합니다.</p>
                ...
                <div key={source} className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
                  ...
                  <div className="mt-2 grid gap-2">
                    {candidates.map((candidate) => {
                      const selected = selectedCoreMetrics.includes(candidate.label);
                      return (
                        <label key={candidate.id} className={`flex min-w-0 gap-3 rounded-2xl border p-3 text-xs font-bold leading-5 ${selected ? 'border-amber-700 bg-white text-amber-950' : 'bg-white/80 text-slate-700'}`}>
                          <input type="checkbox" className="mt-1 shrink-0" checked={selected} onChange={() => toggleMetric(candidate.label)} />
          ...
          <label className="mt-4 block space-y-1">
            ...
            <textarea className="min-h-20 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={extraMetric} onChange={(event) => setExtraMetric(event.target.value)} placeholder="AI 후보에 없지만 꼭 봐야 할 지표가 있으면 짧게 적습니다." />
          </label>
        </div>
      ...
      <div className="rounded-3xl border border-cyan-100 bg-cyan-50 p-5 shadow-sm md:p-6">
        ...
        <p className="mt-2 rounded-2xl bg-white p-3 text-xs font-bold leading-5 text-slate-600">
          앞에서 고른 지표는 팀 전체의 숫자입니다. 이제 그 숫자가 흔들릴 때 어떤 팀원 행동이 보이는지 생각합니다.
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          {coreMetricItems.length === 0 ? <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-500">아직 선택한 핵심 실행 지표가 없습니다</span> : coreMetricItems.map((item) => <span key={item} className="rounded-full bg-white px-3 py-1 text-xs font-black text-cyan-800 shadow-sm">{item}</span>)}
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <label className="block space-y-1">
            <span className="text-xs font-black text-slate-600">왜 이 지표를 보려고 하나요?</span>
            <textarea className="min-h-24 w-full rounded-2xl border bg-white px-3 py-2 text-sm leading-6" value={metricRationale} onChange={(event) => setMetricRationale(event.target.value)} placeholder="예: 방문 수보다 방문 이후 다음 행동이 남았는지를 먼저 보고 싶다." />
          </label>
          <label className="block space-y-1">
            <span className="text-xs font-black text-slate-600">이 지표가 흔들릴 때 보일 팀원 행동은 무엇인가요?</span>
            <textarea className="min-h-24 w-full rounded-2xl border bg-white px-3 py-2 text-sm leading-6" value={behaviorBridge} onChange={(event) => setBehaviorBridge(event.target.value)} placeholder="예: 방문은 했지만 다음 약속, 고객 질문, 기록이 남지 않는다." />
          </label>
        </div>
      </div>
      ...
      <div className="rounded-3xl border bg-white p-5 shadow-sm md:p-6">
        ...
        <h3 className="text-lg font-black text-slate-950">팀원 실행진단: 이 지표에 영향을 주는 사람 신호 찾기</h3>
        <p className="mt-2 rounded-2xl bg-cyan-50 p-3 text-xs font-bold leading-5 text-cyan-900">
          앞에서 선택한 핵심 실행 지표를 떠올리며, 우리 팀에서 비슷한 행동 신호를 보이는 유형 2명을 선택합니다. 사람을 평가하는 것이 아니라, 팀장이 도와야 할 지점을 찾는 단계입니다.
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          {coreMetricItems.map((item) => <span key={item} className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-black text-cyan-800">{item}</span>)}
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          {TEAM_MEMBERS.map((member) => {
            const selected = selectedMemberIds.includes(member.id);
            const disabled = !selected && selectedMemberIds.length >= 2;
            return <MemberSignalCard key={member.id} member={member} selected={selected} disabled={disabled} onToggle={() => toggleMember(member.id)} />;
          })}
        </div>
        ...
        <div className="mt-6 rounded-3xl border bg-slate-50 p-4">
          ...
          {selectedMembers.length === 0 ? (
            <div className="mt-3 rounded-2xl bg-white p-4 text-sm font-bold text-slate-600">먼저 위 카드에서 우리 팀과 비슷한 유형 2명을 선택하세요.</div>
          ) : (
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              {selectedMembers.map((member) => {
                const current = memberPreps[member.id] ?? createEmptyV38MemberPrep();
                return (
                  <article key={member.id} className="rounded-3xl border bg-white p-4">
                    <h5 className="text-base font-black text-slate-950">{member.name}</h5>
                    <label className="mt-3 block space-y-1">
                      <span className="text-xs font-black text-slate-600">실제로 보이는 행동 신호</span>
                      <textarea className="min-h-20 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={current.observedSignal} onChange={(event) => updatePrep(member.id, 'observedSignal', event.target.value)} placeholder="예: 방문은 빠르지만 방문 후 다음 행동 기록이 늦다." />
                    </label>
                    <label className="mt-3 block space-y-1">
                      <span className="text-xs font-black text-slate-600">팀장이 도와야 할 지점</span>
                      <textarea className="min-h-20 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={current.concernSignal} onChange={(event) => updatePrep(member.id, 'concernSignal', event.target.value)} placeholder="예: 방문 후 24시간 안에 다음 행동을 정리하게 돕는다." />
                    </label>
                    <label className="mt-3 block space-y-1">
                      <span className="text-xs font-black text-slate-600">팀원에게 꺼낼 첫마디</span>
                      <textarea className="min-h-20 w-full rounded-2xl border bg-slate-50 px-3 py-2 text-sm leading-6" value={current.finalPrep} onChange={(event) => updatePrep(member.id, 'finalPrep', event.target.value)} placeholder="예: 이번에는 방문 수보다 방문 뒤에 무엇이 남았는지를 같이 보려고 합니다." />
                    </label>
                  </article>
                );
              })}
