...
type PromptConcernBridgeMode = 'research' | 'metric';
...
export function V39PromptConcernBridgeCard({ mode }: { mode: PromptConcernBridgeMode }) {
  ...
  const title = mode === 'research' ? '4단계 AI 전략 리서치 연결' : '5단계 관리 지표 선정 연결';
  const actionLabel = mode === 'research' ? '이번 단계에서는 이렇게 좁힙니다' : '관리 지표로 바꿀 때의 초점';
  ...
  const nextAction = meta?.nextAction || result?.task || '이 고민을 공개자료 리서치 질문과 2주 관리 지표 후보로 바꿉니다.';
