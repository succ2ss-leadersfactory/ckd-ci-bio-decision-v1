...
type HypothesisSource = 'intuition' | 'data' | 'example' | '';
...
type DiagnosisResponse = {
  ...
  selectedHypothesisSource: HypothesisSource;
  promptHypothesis: string;
...
const AI_OUTPUT_OPTIONS: AiOutputOption[] = [
  { id: 'hypothesis-check', title: '가설 타당성 점검', description: '선택한 가설이 데이터 흐름과 맞는지, 과도한 단정은 없는지 점검' },
  { id: 'one-on-one-questions', title: '1on1 확인 질문', description: '팀원에게 실제로 물어볼 질문 3개 제안' },
  { id: 'missed-metrics-risks', title: '놓친 지표/해석 위험', description: '선택하지 않았지만 봐야 할 지표와 해석상 주의점 제시' },
  { id: 'guardrail-check', title: '가드레일 주의사항', description: '컴플라이언스·AI 입력 안전·표현 위험 점검' },
  { id: 'two-week-actions', title: '2주 실행 제안', description: '팀장이 실행할 2주 행동 후보 2~3개 제안', required: true },
  { id: 'leader-script', title: '팀장 대화 스크립트', description: '1on1에서 사용할 부드러운 대화 문장 예시' },
  { id: 'follow-up-metrics', title: '후속 확인 지표', description: '2주 후 무엇을 보고 판단할지 확인 지표와 관찰 방법 제안' },
];
...
export function DashboardAnalysisLab() {
  ...
  const goodExample = useMemo(() => generateGoodExample(currentMember, response), [currentMember, response]);
