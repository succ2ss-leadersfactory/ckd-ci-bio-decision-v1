...
function MetricEvidenceCard({ metric, member, selected, onToggle }: { metric: string; member: Member; selected: boolean; onToggle: () => void }) {
  ...
  return <button type="button" onClick={onToggle} className={`rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow ${selected ? 'border-cyan-500 bg-cyan-50' : metricTone(value)}`}><div className="flex items-start justify-between gap-2"><p className="text-xs font-black text-slate-600">{metric}</p><span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${groupTone(meta.group)}`}>{meta.group} · {meta.expertLabel}</span></div><p className="mt-1 text-2xl font-black text-slate-900">{value}</p><p className="mt-2 text-xs leading-5 text-slate-600">{meta.description}</p></button>;
}
...
function GuardrailEvidenceCard({ metric, member, selected, onToggle }: { metric: string; member: Member; selected: boolean; onToggle: () => void }) {
  ...
  return <button type="button" onClick={onToggle} className={`rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow ${selected ? 'border-cyan-500 bg-cyan-50 text-cyan-900' : guardrailTone(status)}`}><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-black">{metric}</p><p className="mt-1 text-xs font-bold">안전선 점검 · 가드레일</p></div><span className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-black">{status}</span></div><p className="mt-2 text-xs leading-5">{METRIC_META[metric].description}</p></button>;
}
...
export function DashboardAnalysisLab() {
  ...
  return <div className="space-y-4">
    ...
    <SectionCard title="2단계: 데이터로 판단 수정하기">
      ...
      <div className="rounded-2xl border bg-slate-50 p-4 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-black text-slate-900">핵심 근거 지표 선택</p><p className="mt-1 text-xs text-slate-500">각 지표를 별도 카드로 보여줍니다. 선택한 카드는 파란색으로 표시됩니다. 현재 선택: {evidenceCount}개 / 권장: 2~4개</p></div><button className="rounded-xl border bg-white px-3 py-1.5 text-xs font-bold text-slate-700" onClick={() => setShowHints((value) => !value)}>{showHints ? '힌트 숨기기' : '진단 힌트 보기'}</button></div>
        ...
        })}</div></div>)}<div><p className="mb-2 text-xs font-black text-slate-600">안전선 점검 · 가드레일</p><div className="grid gap-3 md:grid-cols-2">{GUARDRAIL_ORDER.map((metric) => <GuardrailEvidenceCard key={metric} metric={metric} member={currentMember} selected={response.selectedGuardrails.includes(metric)} onToggle={() => update({ selectedGuardrails: toggle(response.selectedGuardrails, metric) })} />)}</div></div></div></div>
