...
function csf(id: string, label: string, meaning: string, guide: string, kpis: Kpi[]): Csf {
...
function kpiSet(prefix: string, topic: 'record' | 'followup' | 'question' | 'purpose' | 'blocker' | 'crm' | 'message' | 'boundary' | 'contact' | 'gap'): Kpi[] {
  const common = {
    mid: kpi(`${prefix}-mid-check`, '팀장 중간 점검 질문 작성 건수', '품질', '중간 점검 질문, 후속 확인 질문, 기록 샘플 점검 메모', '팀장이 이번 주에 실제로 물어볼 질문은 무엇인가요?', '질문이 추궁처럼 들리지 않게 관찰 중심으로 표현합니다.'),
    risk: kpi(`${prefix}-risk-expression`, '위험 표현 점검률', '리스크', '수정한 표현, 보류한 답변, 확인 필요 질문', '고객에게 오해를 줄 수 있는 표현은 무엇인가요?', '처방 유도, 경쟁사 비방, 비교 우위 단정 표현을 쓰지 않습니다.'),
  };
  const map: Record<typeof topic, Kpi[]> = {
    record: [
    ...
    purpose: [
      kpi(`${prefix}-visit-purpose`, '방문 전 접점 목적 작성률', '활동', '방문 목적, 확인 질문, 준비 자료, 예상 후속 행동', '방문 전에 무엇을 확인하려는지 명확한가요?', '방문 수 자체를 성과로 단정하지 않습니다.'),
      kpi(`${prefix}-next-topic`, '다음 논의 주제 기록률', '품질', '다음 논의 주제, 고객 질문, 추가 확인 필요 사항', '다음 논의 주제가 고객 질문과 연결되어 있나요?', '고객에게 부담을 주는 후속 논의로 운영하지 않습니다.'),
      kpi(`${prefix}-no-purpose-visit`, '목적 불명확 방문 확인 건수', '리스크', '목적이 약한 반복 방문, 후속 행동 없는 방문', '목적 없는 반복 방문을 줄일 수 있나요?', '활동량 감소를 무조건 부정적으로 보지 않습니다.'),
      common.mid,
    ],
    blocker: [
    ...
    gap: [
      ...
      kpi(`${prefix}-new-contact-record`, '신규 접점 기록률', '품질', '접근 경로, 접점 목적, 고객 반응, 다음 행동', '신규 접점의 목적과 다음 확인 사항이 기록되어 있나요?', '신규 접촉 수 증가만으로 성과를 단정하지 않습니다.'),
...
function csfFor(prefix: string, topic: 'record' | 'followup' | 'question' | 'purpose' | 'blocker' | 'crm' | 'message' | 'boundary' | 'contact' | 'gap', label: string, meaning: string, guide: string): Csf {
  return csf(`${prefix}-${topic}`, label, meaning, guide, kpiSet(`${prefix}-${topic}`, topic));
}
