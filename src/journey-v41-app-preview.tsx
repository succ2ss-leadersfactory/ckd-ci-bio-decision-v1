...
function isParticipantReady(participant: V41Participant) { return Boolean(participant.groupName.trim() && participant.tableName.trim()); }
...
function V41ComplianceNotice() {
  return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><p className="font-black">AI에 넣기 전 확인할 것</p><p className="mt-1">실제 고객, 기관, 사람, 수치, 내부 전략처럼 식별되거나 민감한 정보는 가상·익명 표현으로 바꿉니다.</p></div>;
}
...
function EntryStep({ participant, setParticipant }: { participant: V41Participant; setParticipant: (next: V41Participant) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <V39StepHero eyebrow="1단계 · 시작하기" icon="🚪" title="팀과 이름을 입력하고 바로 시작하세요" tone="indigo" description="오늘 실습은 C1바이오 영업팀장 관점으로 진행됩니다. 먼저 팀과 이름만 입력하면 다음 단계부터 필요한 화면으로 바로 이동할 수 있습니다." badges={[{ label: '필수', value: '팀 / 이름', tone: 'indigo', icon: '✅' }, { label: '역할', value: '영업팀장 관점', tone: 'emerald', icon: '👤' }, { label: '방식', value: '필요 단계 이동', tone: 'amber', icon: '🧭' }]} />
    <section className="rounded-3xl border border-indigo-100 bg-white p-4 shadow-sm md:p-5"><div className="grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-indigo-50 p-4"><p className="text-sm font-black text-indigo-950">1. 팀 선택</p><p className="mt-1 text-xs font-bold leading-5 text-indigo-800">조 활동 기준으로 선택합니다.</p></div><div className="rounded-2xl bg-emerald-50 p-4"><p className="text-sm font-black text-emerald-950">2. 이름 입력</p><p className="mt-1 text-xs font-bold leading-5 text-emerald-800">닉네임이나 테이블명도 괜찮습니다.</p></div><div className="rounded-2xl bg-amber-50 p-4"><p className="text-sm font-black text-amber-950">3. 바로 이동</p><p className="mt-1 text-xs font-bold leading-5 text-amber-800">2단계 이후에는 필요한 화면으로 이동합니다.</p></div></div></section>
    <V41ComplianceNotice />
    <ShellCard title="시작 정보 입력"><div className="grid gap-3 md:grid-cols-2"><label className="space-y-1"><span className="text-xs font-bold text-slate-500">팀 <span className="text-rose-600">필수</span></span><select className="w-full rounded-xl border bg-white px-3 py-2" value={participant.groupName} onChange={(event) => setParticipant({ ...participant, groupName: event.target.value })}><option value="">팀을 선택하세요</option>{TEAM_OPTIONS.map((team) => <option key={team} value={team}>{team}</option>)}</select></label><label className="space-y-1"><span className="text-xs font-bold text-slate-500">이름/닉네임 <span className="text-rose-600">필수</span></span><input className="w-full rounded-xl border bg-white px-3 py-2" value={participant.tableName} onChange={(event) => setParticipant({ ...participant, tableName: event.target.value })} placeholder="예: 김팀장, A테이블, 리더1" /></label></div><label className="block space-y-1"><span className="text-xs font-bold text-slate-500">오늘 다뤄보고 싶은 상황 <span className="text-slate-400">선택</span></span><textarea className="min-h-20 w-full rounded-xl border px-3 py-2" value={participant.representativeSituation} onChange={(event) => setParticipant({ ...participant, representativeSituation: event.target.value })} placeholder="예: 고객 반응은 기록되는데 다음 행동으로 잘 이어지지 않는다." /></label><label className="flex items-start gap-2 rounded-xl bg-slate-50 p-3"><input className="mt-1" type="checkbox" checked={participant.roleAccepted} onChange={(event) => setParticipant({ ...participant, roleAccepted: event.target.checked })} /><span>AI 결과는 답이 아니라 비교하고 고쳐 쓸 초안으로 다룹니다.</span></label>{!ready ? <p className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">팀과 이름/닉네임을 입력하면 다음 단계로 이동할 수 있습니다.</p> : null}</ShellCard>
  </div>;
}

function RoleTeamIntroStep() {
  return <div className="space-y-4"><V41FlowStrip currentStep={2} /><V39StepHero eyebrow="2단계 · 팀원 보기" icon="👥" title="오늘 함께 볼 팀원 7명입니다" tone="indigo" description="이대호 팀장 관점에서 팀원별 실행 신호를 가볍게 확인합니다. 지금은 평가가 아니라 이후 1on1과 업무관리에서 참고할 관찰 자료를 보는 단계입니다." badges={[{ label: '역할', value: '이대호 팀장', tone: 'indigo', icon: '👤' }, { label: '팀원', value: '7명', tone: 'emerald', icon: '👥' }, { label: '관점', value: '관찰 먼저', tone: 'amber', icon: '🔎' }]} /><section className="grid gap-3 md:grid-cols-3"><div className="rounded-3xl border border-cyan-100 bg-cyan-50 p-4"><p className="text-sm font-black text-cyan-950">먼저 볼 것</p><p className="mt-2 text-sm leading-6 text-cyan-900">팀원별로 어떤 실행 신호가 보이는지 확인합니다.</p></div><div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4"><p className="text-sm font-black text-emerald-950">아직 하지 않을 것</p><p className="mt-2 text-sm leading-6 text-emerald-900">성급하게 평가하거나 원인을 단정하지 않습니다.</p></div><div className="rounded-3xl border border-amber-100 bg-amber-50 p-4"><p className="text-sm font-black text-amber-950">나중에 쓸 곳</p><p className="mt-2 text-sm leading-6 text-amber-900">10·11단계에서 1on1 대상과 첫 문장을 준비할 때 다시 활용합니다.</p></div></section><ShellCard title="팀원별 실행 신호"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{TEAM_MEMBERS.map((member) => <div key={member.name} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="font-black text-slate-950">{member.name}</p><p className="mt-1 text-xs font-bold text-slate-500">{member.role}</p><p className="mt-2 text-sm leading-6 text-slate-700">{member.signal}</p></div>)}</div></ShellCard></div>;
}
