...
export function V41PerformanceAiExpansionLab() {
  ...
  const buildTeamTaskPrompt = () => {
    const prompt = [
      ...
      '',
      `[상황/맥락]\n전사 전략과제: ${enterpriseTitle}\n현재 선택된 팀 전략과제: ${baseTeamTask}\n현재 선택된 CSF: ${baseCsf}\n현재 선택된 KPI: ${baseKpi}\n현재 세부 추진과제 후보: ${baseInitiative}`,
      '',
      '[과제/요청]',
      '현재 선택된 팀 기준을 유지하되, 보완적으로 검토할 추가 팀 전략과제 후보 5개를 제안해 주세요.',
      '',
      '[출력형식]',
      '1. 추가 팀 전략과제 후보 5개',
      '2. 각 후보의 전사 전략 기여도',
      '3. 현장 실행 난이도',
      '4. 컴플라이언스 주의점',
      '5. 추천 우선순위와 제외해도 되는 후보',
      '',
      '[제약/조건]',
      '실제 병원명, 의료진명, 고객명은 쓰지 마세요. 처방 확대, 의사 설득, 경쟁 제품 차단처럼 위험하게 해석될 수 있는 표현은 피하세요. 팀장이 현장에서 쓸 수 있는 말로 작성해 주세요.',
    ].join('\n');
  ...
  const buildCsfPrompt = () => {
    ...
    const prompt = [
      ...
      '',
      '[과제/요청]',
      '선정된 추가 팀 전략과제가 성공하려면 반드시 충족되어야 할 CSF 후보 5개를 제안해 주세요. 단순 활동이 아니라 성공조건으로 작성해 주세요.',
      '',
      '[출력형식]',
      '1. CSF 후보 5개',
      '2. 각 CSF가 성공조건인 이유',
      ...
      '4. 현장에서 확인 가능한 증거',
      '5. 최종 선택 추천 1~2개와 제외 후보',
      '',
      '[제약/조건]',
      'CSF는 방문 수, 자료 전달 수 같은 단순 활동으로 쓰지 마세요. 실제 병원명, 의료진명, 고객명은 쓰지 마세요. 컴플라이언스 위험 표현은 피하세요.',
    ].join('\n');
  ...
  const buildKpiPrompt = () => {
    ...
    const prompt = [
      ...
      '',
      '[과제/요청]',
      '선정된 CSF를 실제로 확인할 수 있는 KPI 후보 5개를 제안해 주세요. 각 KPI는 확인 증거, 관리 주기, 팀원 입력 가능성, 팀장 점검 가능성을 포함해야 합니다.',
      '',
      '[출력형식]',
      '1. KPI 후보 5개',
      ...
      '',
      '[제약/조건]',
      '확인 가능한 증거가 없는 KPI는 제외해 주세요. 단순 활동량 지표만 제안하지 마세요. 실제 병원명, 의료진명, 고객명은 쓰지 마세요. 위험 표현은 피하세요.',
    ].join('\n');
  ...
  return <section className="rounded-3xl border border-violet-100 bg-violet-50 p-4 shadow-sm md:p-5" data-v41-ai-expansion-position="before-team-standard-confirmation">
    ...
    <p className="mt-2 text-sm font-bold leading-6 text-slate-600">AI가 한 번에 답을 정하게 하지 않고, 추가 팀 전략과제 → CSF → KPI 순서로 후보를 만들고 팀장이 단계별로 선택합니다. 최종 검토 요약만 6단계 실행계획 후보로 연결됩니다.</p>
    <div className="mt-3 flex flex-wrap gap-2 text-xs font-black text-violet-800"><span className="rounded-full bg-white px-3 py-1">전략과제</span><span className="rounded-full bg-white px-3 py-1">CSF</span><span className="rounded-full bg-white px-3 py-1">KPI</span><span className="rounded-full bg-white px-3 py-1">사람 선택</span><span className="rounded-full bg-white px-3 py-1">6단계 연결</span></div>

    <div className="mt-4 grid gap-4">
      <StageCard step="1단계" title="추가 팀 전략과제 도출 및 선정" help="전사 전략과 현재 팀 기준을 바탕으로 AI가 추가 팀 전략과제 후보를 만들고, 팀장이 1개를 선정합니다.">
      ...
      <StageCard step="2단계" title="선정된 전략과제별 CSF 도출 및 선정" help="1단계에서 선정한 전략과제를 기준으로 CSF 후보를 만들고, 성공조건으로 적합한 항목을 선정합니다.">
      ...
      <StageCard step="3단계" title="선정된 CSF별 KPI 도출 및 선정" help="선정한 CSF를 실제로 확인할 수 있는 KPI, 확인 증거, 관리 주기를 확정합니다.">
      ...
      <section className="rounded-3xl border border-violet-100 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-violet-100 bg-violet-50 p-4 text-xs font-bold leading-5 text-violet-950"><p className="font-black">6단계 연결 원칙</p><p className="mt-2">1. AI 원문은 참고자료입니다.</p><p>2. 단계별로 사람이 선택한 전략과제·CSF·KPI만 전달합니다.</p><p>3. 확인 증거와 관리 주기가 없는 KPI는 6단계 실행계획에 넣지 않습니다.</p><p>4. 컴플라이언스 위험 표현은 제거하거나 수정합니다.</p><p>5. 최종 요약이 있어야 6단계 반영 메모를 만들 수 있습니다.</p></div>
        </div>
      </section>
    </div>
  </section>;
