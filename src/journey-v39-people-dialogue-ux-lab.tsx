    ...
    <section className="space-y-4">
      <section className="rounded-3xl border border-violet-100 bg-white p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-wide text-violet-700">People Dialogue UX Guide</p>
            <h2 className="mt-2 text-2xl font-black text-slate-950">9단계 진행 가이드</h2>
            <p className="mt-2 max-w-4xl text-sm font-bold leading-6 text-slate-600">
              이 단계는 팀원을 평가하거나 세대 차이를 단정하는 단계가 아닙니다. 팀장이 실제로 마주치는 대화 상황을 고르고,
              평소 첫마디가 어떻게 들릴 수 있는지 확인한 뒤, 목적에 맞는 실행 대화로 바꾸는 단계입니다.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 lg:w-[26rem]">
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3">
              ...
              <p className="mt-1 text-[11px] font-bold leading-4 text-emerald-800">{savedRoleCount > 0 ? '이전 단계 역할 방향을 대화 맥락으로 참고합니다.' : '8단계 저장 결과가 있으면 이곳에 반영됩니다.'}</p>
            </div>
            <div className="rounded-2xl border border-violet-100 bg-violet-50 px-4 py-3">
              ...
              <p className="mt-1 text-[11px] font-bold leading-4 text-violet-800">작성 내용은 10단계 실행계획 프롬프트에 반영됩니다.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-violet-100 bg-violet-50 p-4 text-xs font-bold leading-5 text-violet-950">
            ...
            <p className="mt-1">대화 상황을 선택하고, 평소 첫마디를 목적에 맞는 실행 대화로 바꿉니다.</p>
          </div>
          <div className="rounded-2xl border border-sky-100 bg-sky-50 p-4 text-xs font-bold leading-5 text-sky-950">
            <p className="font-black">이전 단계에서 가져온 것</p>
            <p className="mt-1">8단계 팀원 역할 방향, 코칭 초점, 리스크 안전선입니다.</p>
          </div>
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-xs font-bold leading-5 text-emerald-950">
            <p className="font-black">다음 단계로 넘길 것</p>
            <p className="mt-1">10단계 AI 실행계획 프롬프트에 반영할 대화 목적과 첫마디입니다.</p>
          </div>
        </div>

        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold leading-5 text-slate-700">
          최소 결과물은 <span className="font-black text-slate-950">대화 상황 선택</span>, <span className="font-black text-slate-950">대화 목적 선택</span>, <span className="font-black text-slate-950">평소 첫마디 확인</span>, <span className="font-black text-slate-950">실제로 사용할 첫 문장 저장</span>입니다.
          선택 입력은 시간이 있을 때 보완해도 됩니다.
        </div>
      </section>
