...
export function CustomerCallPlanLab() {
  ...
  const updateClassification = (groupId: string, value: string) => {
    update({ groupClassifications: { ...response.groupClassifications, [groupId]: value } });
  };
  const updateReason = (groupId: string, value: string) => {
    update({ classificationReasons: { ...response.classificationReasons, [groupId]: value } });
  };
  ...
  return <div className="space-y-4">
    ...
    <SectionCard title="상황 제시: 고객 Data를 보고 2주 콜플랜 방향 정하기">
      ...
      <div className="rounded-xl bg-cyan-50 p-4 text-sm text-cyan-900"><p className="font-bold">오늘의 판단 과제</p><p className="mt-1">고객 Data → 고객군 분류 → 집중/후순위 판단 → AI 결과물 요청 → 2주 콜플랜 카드 확정</p></div>
    </SectionCard>

    <SectionCard title="1단계: 고객 묶음별 현업형 Data 읽기">
      <div className="grid gap-3">
        {CUSTOMER_GROUPS.map((group) => <article key={group.id} className="rounded-2xl border bg-slate-50 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-black text-slate-900">{group.label}</h4><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600">추천 예시: {group.recommendedType}</span></div>
          <p className="mt-2 text-sm leading-6 text-slate-700">{group.dataSignal}</p>
          <div className="mt-3 grid gap-2 md:grid-cols-4">
            <DataPill label="고객 등급" value={group.customerGrade} />
            <DataPill label="잠재력" value={group.potentialGrade} />
            <DataPill label="관계 수준" value={group.relationshipLevel} />
            <DataPill label="최근 방문일" value={group.lastVisit} />
            <DataPill label="4주 콜 횟수" value={`${group.callCount4w}회`} />
            <DataPill label="접촉 성공률" value={group.contactSuccessRate} />
            <DataPill label="최근 콜 반응" value={group.recentCallReaction} />
            <DataPill label="자료 요청" value={group.materialRequest} />
            <DataPill label="후속 미팅" value={group.followUpMeeting} />
            <DataPill label="거절/보류 사유" value={group.holdReason} />
            <DataPill label="후속조치 완료율" value={group.followUpCompletion} />
            <DataPill label="CRM 기록" value={group.crmQuality} />
            <DataPill label="컴플라이언스" value={group.complianceSensitivity} />
          </div>
        </article>)}
      </div>
    </SectionCard>
    ...
    <SectionCard title="2단계: 고객군 직접 분류하기">
      ...
      <div className="space-y-3">
        {CUSTOMER_GROUPS.map((group) => <div key={group.id} className="rounded-2xl border p-4">
          <div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-slate-900">{group.label}</p><span className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-800">{group.dataSignal}</span></div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <label className="space-y-1"><FieldLabel>분류 유형</FieldLabel><select className="w-full rounded-xl border px-3 py-2" value={response.groupClassifications[group.id] || ''} onChange={(event) => updateClassification(group.id, event.target.value)}><option value="">선택하세요</option>{CUSTOMER_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
            <label className="space-y-1"><FieldLabel>분류 이유</FieldLabel><input className="w-full rounded-xl border px-3 py-2" value={response.classificationReasons[group.id] || ''} onChange={(event) => updateReason(group.id, event.target.value)} placeholder="예: 반응 상승과 후속 미팅 동의가 있어 집중군으로 판단" /></label>
          </div>
        </div>)}
      </div>
    ...
    <SectionCard title="3단계: 집중/후순위 고객군 판단하기">
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1"><FieldLabel>2주 집중 고객군 유형</FieldLabel><select className="w-full rounded-xl border px-3 py-2" value={response.selectedFocusType} onChange={(event) => update({ selectedFocusType: event.target.value })}><option value="">선택하세요</option>{CUSTOMER_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
        <label className="space-y-1"><FieldLabel>후순위 고객군 유형</FieldLabel><select className="w-full rounded-xl border px-3 py-2" value={response.selectedDeprioritizedType} onChange={(event) => update({ selectedDeprioritizedType: event.target.value })}><option value="">선택하세요</option>{CUSTOMER_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-xl bg-cyan-50 p-3 text-sm text-cyan-900"><b>집중 고객 묶음:</b> {getGroupsByType(response, response.selectedFocusType)}</div>
        <div className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900"><b>후순위 고객 묶음:</b> {getGroupsByType(response, response.selectedDeprioritizedType)}</div>
      </div>
    ...
    <SectionCard title="6단계: 2주 콜플랜 카드 확정하기">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1"><FieldLabel>2주 집중 방향</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalFocusDirection} onChange={(event) => update({ finalFocusDirection: event.target.value })} /></label>
        <label className="space-y-1"><FieldLabel>고객군별 운영 원칙</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalOperatingPrinciples} onChange={(event) => update({ finalOperatingPrinciples: event.target.value })} /></label>
        <label className="space-y-1"><FieldLabel>팀원별 역할</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalMemberRoles} onChange={(event) => update({ finalMemberRoles: event.target.value })} /></label>
        <label className="space-y-1"><FieldLabel>후속조치·CRM 기준</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalFollowUpStandards} onChange={(event) => update({ finalFollowUpStandards: event.target.value })} /></label>
        <label className="space-y-1"><FieldLabel>하지 않을 행동/주의 표현</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalAvoidActions} onChange={(event) => update({ finalAvoidActions: event.target.value })} /></label>
        <label className="space-y-1"><FieldLabel>팀장 회의 공유 문장</FieldLabel><textarea className="min-h-24 w-full rounded-xl border px-3 py-2" value={response.finalManagerMessage} onChange={(event) => update({ finalManagerMessage: event.target.value })} /></label>
      </div>
